...
<svelte:head>
	<title>Sign Up — Session Master</title>
</svelte:head>
...
<div class="min-h-screen bg-stone-950 flex items-center justify-center p-4">
	<div class="w-full max-w-md">
		<div class="text-center mb-8">
			...
			<p class="text-stone-400 mt-2">Begin your adventure</p>
		</div>

		<form onsubmit={handleSignup} class="bg-stone-900 rounded-lg p-8 shadow-lg border border-stone-800">
			{#if error}
			...
			<div class="space-y-5">
				<div>
					<label for="displayName" class="block text-sm font-medium text-stone-300 mb-1.5">Display Name</label>
					<input id="displayName" type="text" bind:value={displayName} required placeholder="Shadowheart"
		...
		<p class="text-center text-stone-500 mt-6 text-sm">
			Already have an account? <a href="/login" class="text-amber-500 hover:text-amber-400">Log in</a>
		</p>
	</div>
</div>
