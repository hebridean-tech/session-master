...
import { extractEntities } from '$lib/ai/features';
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    const result = await extractEntities(noteId, tableId, session.user.id);
    return json({ success: true, entities: result.entities, jobId: result.jobId });
  } catch (e: any) {
